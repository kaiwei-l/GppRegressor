...
def site_sanitizer_aux(siteDataPath, modisPath, sitelistdf, outputpath, scale, algoName):
    ...
    df['Date'] = df.apply(lambda dfrow: str(int(dfrow['TIMESTAMP_START']))[:8], axis=1)  # Serve as key to be merged with MODIS df

    # Merge dataframe
    print("III. Merging...")
    concat_df = df.join(modisdf.set_index('Date'), on='Date')
    concat_df.dropna(inplace=True)
    new_fname = scale + "_" + algoName + "_" + siteid + ".csv"
    concat_df.to_csv(os.path.join(outputpath, new_fname), index=False)
...
scale_list = ['3km', '5km']
algo_list = ['MCD43A4', 'MOD', 'MYD']
# for scale in scale_list:
#     for algo in algo_list:
#         site_sanitizer(inputPath, outputPath, modisPath, scale, algo)
